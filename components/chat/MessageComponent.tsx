...
const MessageComponent: React.FC<MessageComponentProps> = ({ sender, text, timestamp }) => {
  ...
  return (
    <div className={`flex items-end ${isUser ? "justify-end" : "justify-start"} gap-2`}>
      ...
      <Card className={`px-3 py-1 max-w-xs md:max-w-md lg:max-w-lg break-words shadow-sm gap-0 flex flex-col ${isUser ? "bg-primary text-primary-foreground" : "bg-muted"}`}>
